...
var w = (c.width = winW),
  h = (c.height = winH * 2), // كما كان في كودك الأصلي
  ctx = c.getContext("2d"),
  hw = w / 2, 
  hh = winH * 1.5, // مركز الانفجار الصحيح بناءً على تصميمك
  ...
  opts = {
    ...
    charSize: isMobile ? 26 : 40,       // 💡 تم تكبير الخط ليكون واضحاً وجميلاً
    charSpacing: isMobile ? 28 : 45,    // 💡 مسافة مناسبة تمنع التداخل
    lineHeight: isMobile ? 35 : 55,     // 💡 مسافة مناسبة بين السطور
...
function anim() {
  ...
  for (var l = 0; l < letters.length; ++l) {
    letters[l].step();
    if (letters[l].phase === "balloon" && !letters[l].spawning && !letters[l].inflating) {
       startedFlying = true;
    }
  }
  ...
  if (startedFlying && window.startBirthdayTransition) {
     window.startBirthdayTransition();
  }
}

for (var i = 0; i < opts.strings.length; ++i) {
  var chars = Array.from(opts.strings[i]); 
  
  for (var j = 0; j < chars.length; ++j) {
    letters.push(
      new Letter(
        chars[j],
        // 💡 التعديل الوحيد: هذه المعادلة تضمن عدم تداخل الحروف معاً أبداً
...
window.startFireworksAnim = function() { anim(); };
